...
module.exports = class WeakFunctionPool extends FunctionResolver {
  ...
  async onmessage(message) {
    ...
    switch (message.type) {
          ...
          this.messageBuilder("executeResponse", message.requestId, {
            ...
            payload: this.serializeObject(
              await this.executeFunctionCatcher(
                ...
                ...this.deSerializeObject(message.data.payload, this.setTheirs)
              ),
      ...
      case "executeResponse":
        ...
        Reflect.deleteProperty(this.theirsFunctionsWaitPool, message.requestId);
        this.logger.silly(
    ...
    const resolveFunction = (...params) =>
      new Promise((resolve, reject) => {
        ...
        this.theirsFunctionsWaitPool[requestId] = (response) => {
          this.logger.silly("Got Response by function response Handler", id);
          if (
        ...
        this.sendMessage(
          this.messageBuilder("execute", requestId, {
            id,
            payload: this.serializeObject(params, this.setOurs),
          })
        );
        this.logger.silly(
  ...
  getTheirs(id) {
    return this.theirsFunctions.has(id) ? this.theirsFunctions.get(id) : null;
  }
};
